...
function Inv.Util.CheckRegisterInventoryParams(name, metadata, overwrite)

    if Inv.Util.StringEmptyOrNull(name) then 
        Inv.Util.LogWarning("RegisterInventory", "The given name for this inventory was null or empty. got: " .. tostring(name))
        return false, 100 
    end

    if Inv.Util.IsNull(metadata) then 
        Inv.Util.LogWarning("RegisterInventory", "The given metadata for " .. name .. " was empty or nil. got: " .. tostring(metadata))
        return false, 101 
    end

    if not Inv.Util.IsNumber(metadata.width, 1, 255) then 
        Inv.Util.LogWarning("RegisterInventory", "The width metadata for inventory " .. name .. " was not a number, below 1 or higher than 255. got: " .. tostring(metadata.width))
        return false, 102 
    end

    if not Inv.Util.IsNumber(metadata.height, 1, 255) then 
        Inv.Util.LogWarning("RegisterInventory", "The height metadata for inventory " .. name .. " was not a number, below 1 or higher than 255. got: " .. tostring(metadata.height))
        return false, 103 
    end

    if not Inv.Util.IsNumber(metadata.theme, 0, 255) then 
        Inv.Util.LogWarning("RegisterInventory", "The theme metadata for inventory " .. name .. " was not a number, below 0 or higher than 255. got: " .. tostring(metadata.theme))
        return 104 
    end

    if not Inv.Util.IsNumber(metadata.style, 0, 255) then 
        Inv.Util.LogWarning("RegisterInventory", "The style metadata for inventory " .. name .. " was not a number, below 0 or higher than 255. got: " .. tostring(metadata.style))
        return 105 
    end

    if not Inv.Util.StringOrNull(metadata.owner) then 
        Inv.Util.LogWarning("RegisterInventory", "The owner metadata for inventory " .. name .. " was not nil or a string. got: " .. tostring(metadata.owner))
        return 106 
    else
        if not Inv.Util.IsNull(metadata.owner) then
            print(1)
            if not Inv.Util.StringIsOfSize(metadata.owner, 1, 64) then 
                Inv.Util.LogWarning("RegisterInventory", "The owner metadata for inventory " .. name .. " cannot have more than 64 characters. got: " .. tostring(metadata.owner))
                return 107 
            end
        end
    end
    ...
    if not Inv.Util.IsBool(metadata.transient) then 
        Inv.Util.LogWarning("RegisterInventory", "The transient metadata for inventory " .. name .. " is not a boolean. got: " .. tostring(metadata.transient))
        return 108 
    end

    if metadata.transient and metadata.owner ~= nil then 
        Inv.Util.LogWarning("RegisterInventory", "Invalid state for inventory " .. name .. ", transient inventories cannot have a owner (because they can't possibly belong to anyone).")
        return 110 
    end
